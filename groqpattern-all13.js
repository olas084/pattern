...
const SEARCH_STATE_FILE = path.join(RESULTS_DIR, "search-state.json");
...
const GROQ_RESUME_ENABLED = ["1", "true", "yes", "on"].includes(String(process.env.GROQ_RESUME_ENABLED ?? "true").trim().toLowerCase());
...
const healthServer = http.createServer((req, res) => {
  ...
  res.end("groq-all13-runner alive");
});
...
function computeCrossStats(rawCandles, higherTF, higherBarsInput) {
  const higherBars = higherBarsInput || resample(rawCandles, higherTF);
...
function buildHigherBarsMap(rawCandles) {
  const higherBarsMap = {};
  for (const tf of TIMEFRAMES) {
    if (tf > 1) higherBarsMap[tf] = resample(rawCandles, tf);
  }
  return higherBarsMap;
}
...
async function loadResumeState(expectedSymbols) {
  if (!GROQ_RESUME_ENABLED) return null;

  // 1) Try Mongo latest run first.
  if (mongoose && MONGO_URI) {
    try {
      ...
      const latest = await GroqUniversalRun.findOne({ workflow: "GROQ_UNIVERSAL_ALL13_SIDE_SEPARATED_WALK" })
        .sort({ timestamp: -1 })
        .lean();
      await mongoose.connection.close();
      if (latest && sameSymbols(latest.symbols_analyzed, expectedSymbols)) {
        console.log("Resume state loaded from MongoDB latest run.");
        return latest;
      }
      if (latest) {
        console.log("Mongo resume state ignored: symbols list mismatch.");
      }
    } catch (err) {
      console.warn(`Resume from MongoDB skipped: ${err.message}`);
      try { await mongoose.connection.close(); } catch (_) {}
    }
  }

  // 2) Fallback to local search-state file.
  try {
    if (fs.existsSync(SEARCH_STATE_FILE)) {
      const local = JSON.parse(fs.readFileSync(SEARCH_STATE_FILE, "utf8"));
      const localSymbols = Array.isArray(local.symbols_analyzed) ? local.symbols_analyzed : expectedSymbols;
      if (sameSymbols(localSymbols, expectedSymbols)) {
        console.log("Resume state loaded from local search-state.json.");
        return local;
      }
      console.log("Local resume state ignored: symbols list mismatch.");
    }
  } catch (err) {
    console.warn(`Local resume read failed: ${err.message}`);
  }
...
async function run() {
  ...
  for (const symbol of SYMBOLS) {
    ...
    const crossStats = TIMEFRAMES.filter((tf) => tf > 1).map((tf) => computeCrossStats(raw, tf, resampled[tf]));
    symbolBundles.push({ symbol, rawCount: raw.length, tfStats, crossStats });
  ...
  async function getSymbolRuntime(symbol) {
    const existing = runtimeCache.get(symbol);
    if (existing) return existing;
    const raw = await loadRawCandles(db, symbol, candleTable);
    const runtime = { raw, higherBarsMap: buildHigherBarsMap(raw) };
    runtimeCache.set(symbol, runtime);
    // Keep only a tiny runtime cache to control memory on small instances.
    const MAX_RUNTIME_CACHE = 2;
    while (runtimeCache.size > MAX_RUNTIME_CACHE) {
      const oldestKey = runtimeCache.keys().next().value;
      if (!oldestKey || oldestKey === symbol) break;
  ...
  // Resume memory so failed/success patterns are not reused after restart.
  const resumeState = await loadResumeState(symbolBundles.map((b) => b.symbol));
  if (resumeState) {
    const sps = Array.isArray(resumeState.successful_patterns) ? resumeState.successful_patterns : [];
    const fps = Array.isArray(resumeState.failed_patterns) ? resumeState.failed_patterns : [];
    const aps = Array.isArray(resumeState.active_patterns_left)
      ? resumeState.active_patterns_left
      : (Array.isArray(resumeState.active_patterns_full) ? resumeState.active_patterns_full : []);
    const atts = Array.isArray(resumeState.attempts) ? resumeState.attempts : [];

    successfulPatterns.push(...sps);
    failedPatterns.push(...fps);
    activePatterns.push(...aps);
    attempts.push(...atts);
    tokenUsage = resumeState.token_usage && typeof resumeState.token_usage === "object"
      ? { prompt: Number(resumeState.token_usage.prompt || 0), output: Number(resumeState.token_usage.output || 0) }
      : tokenUsage;
    startCycle = Math.max(1, Number(resumeState.cycles_run || 0) + 1);

    // Rebuild tested keys from all memory pools.
    for (const p of successfulPatterns) if (p?.pattern_key) testedKeys.add(p.pattern_key);
    for (const p of failedPatterns) if (p?.pattern_key) testedKeys.add(p.pattern_key);
    for (const p of activePatterns) if (p?.pattern_key) testedKeys.add(p.pattern_key);

    // Continue generation numbering from previous attempts.
    const genLabels = attempts
      .map((a) => String(a?.label || ""))
      .map((x) => {
        const m = x.match(/gen-pair-(\d+)/);
        return m ? Number(m[1]) : 0;
      });
    generationId = genLabels.length ? Math.max(...genLabels) : 0;

    console.log(
      `Resumed: cycle=${startCycle}, successful=${successfulPatterns.length}, failed=${failedPatterns.length}, active=${activePatterns.length}`
    );
  }
  ...
  async function generateCandidatePair(processedSymbolCount) {
    ...
    for (const side of ["BUY", "SELL"]) {
      ...
      for (let i = 0; i < processedSymbolCount; i++) {
        ...
        const runtime = await getSymbolRuntime(b.symbol);
        const stats = testPattern(runtime.raw, candidate, runtime.higherBarsMap);
  ...
  for (let cycle = startCycle; cycle <= MAX_CYCLES; cycle++) {
    ...
    for (let symbolIdx = 0; symbolIdx < symbolBundles.length; symbolIdx++) {
      ...
      for (const ap of activeNow) {
        ...
        const runtime = await getSymbolRuntime(b.symbol);
        const stats = testPattern(runtime.raw, stillActive.pattern, runtime.higherBarsMap);
        const row = createRowForSymbol(stats, b.symbol);
    ...
    fs.writeFileSync(SEARCH_STATE_FILE, JSON.stringify({
      ...
      active_patterns_full: activePatterns,
      attempts,
